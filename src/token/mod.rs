...
const KEYWORDS: &'static [(&'static str, Token<'static>)] = &[
    ...
    ("CREATE", Create),
    ("CROSS", JoinKw),
    ("CURRENT_DATE", CtimeKw),
    ("CURRENT_TIME", CtimeKw),
    ("CURRENT_TIMESTAMP", CtimeKw),
    ("DATABASE", Database),
    ("DEFAULT", Default),
...
impl<'input> Tokenizer<'input> {
    ...
    fn next_unshifted(&mut self) -> Option<Result<Spanned<Token<'input>>, Error>> {
        loop {
            return match self.lookahead {
                ...
                Some((idx0, c)) if c == '`' || c == '\'' || c == '"' => {
                    Some(self.literal(idx0, c))
                }
                Some((idx0, '.')) => {
                    match self.bump() {
                ...
                Some((idx0, _)) => Some(error(UnrecognizedToken, idx0, self.text)),
                None => None
            };
        }
    }
    ...
    fn literal(&mut self, idx0: usize, delim: char) -> Result<Spanned<Token<'input>>, Error> {
        let mut pc = '\0';
        loop {
            match self.bump() {
                Some((idx1, c)) if c == delim && pc => {
    ...
    fn blob_literal(&mut self, idx0: usize) -> Result<Spanned<Token<'input>>, Error> {
        let mut n = 0;
        loop {
            match self.bump() {
    ...
    fn fractional_part(&mut self, idx0: usize) -> Result<Spanned<Token<'input>>, Error> {
    ...
    fn exponential_part(&mut self, idx0: usize) -> Result<Spanned<Token<'input>>, Error> {
        match self.bump() {
            Some((_, '+')) | Some((_, '-')) => {
                self.bump();
            }
    ...
    fn number(&mut self, idx0: usize, digit: char) -> Result<Spanned<Token<'input>>, Error> {
        if digit == '0' {
            match self.bump() {
                Some((_, 'x')) | Some((_, 'X')) => {
                    self.bump();
    ...
    fn hex_integer(&mut self, idx0: usize) -> Result<Spanned<Token<'input>>, Error> {
        match self.take_while_1(|c| c.is_digit(10)) {
    ...
    fn bracket(&mut self, idx0: usize) -> Result<Spanned<Token<'input>>, Error> {
        match self.take_until(|c| c == ']') {
            Some(idx1) => {
                self.bump();
                let id: &'input str = &self.text[idx0 + 1..idx1];
                Ok((idx0, Id(id), idx1 + 1))
            }
...
fn is_identifier_start(c: char) -> bool {
    (c >= 'A' && c <= 'Z') || (c >= 'a' && c <= 'z') || c == '_' || c > '\x7F'
}

fn is_identifier_continue(c: char) -> bool {
    c == '$' || (c >= '0' && c <= '9') || (c >= 'A' && c <= 'Z') || c == '_' || (c >= 'a' && c <= 'z') || c > '\x7f'
}
